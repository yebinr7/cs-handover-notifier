...
- **주기**: 상시 켜진 서버나 PC 없이, GitHub Actions가 5분마다 저장소를 체크아웃해서 `notify.py`를 대신 실행해줍니다.
- **감지 방식**: 마지막으로 확인한 시각(`state.json`의 `last_checked`) 이후에 "인수인계" DB에 새로 생성된 페이지가 있는지 Notion API로 조회합니다. 새 글이 없으면 아무 일도 안 하고 조용히 끝납니다.
- **"요약"이 어떻게 만들어지나요**: AI가 내용을 알아서 요약해주는 게 아닙니다. 노션 페이지의 **"요약"과 "체크할것" 속성에 직접 적어놓은 텍스트를 그대로** 가져와서 텔레그램 메시지에 옮겨 담을 뿐입니다. 즉 노션에 뭘 적느냐가 곧 텔레그램에 오는 내용입니다 — 요약이나 체크할것을 비워두면 그 줄은 메시지에서 아예 빠집니다.
- **가져오는 항목**: "이름"(제목, 예: "야간 김예빈"), "요약", "체크할것", 그리고 해당 노션 페이지로 바로 이동하는 링크. "생성자" 속성은 메시지에 별도로 포함되지 않습니다 — "이름"에 이미 담당자가 들어있는 구조라서입니다.
- **메시지 예시**:
  ```
  🌙 야간 김예빈 (2026-09-08)
  요약: 서보 다 전원 나간 이유..
  ...
  - 날짜는 한국 시간(KST) 기준으로 표시됩니다 (Notion은 내부적으로 UTC로 저장하지만 변환해서 보여줍니다).
  - 요약/체크할것 내용이 1000자를 넘으면 텔레그램 메시지 길이 제한 때문에 뒷부분이 `…`로 잘려서 표시됩니다.
- **여러 건이 한꺼번에 생겼을 때**: 오래된 글부터 순서대로 하나씩 보냅니다. 중간에 전송이 실패하면(예: 텔레그램 서버 일시 장애) 그 지점에서 멈추고, 실패한 글과 그 뒤에 밀려있던 글들은 전부 다음 5분 주기에 다시 시도합니다 — 이미 보낸 글이 유실되는 일은 없습니다.
