...
- **"요약"이 어떻게 만들어지나요**: "요약"/"체크할것" 속성은 사람이 대충 적는 메모라 그대로 보내기엔 부족합니다. 대신 그 노션 페이지의 **본문**(문단, 목록, 체크박스 등에 적힌 실제 내용)을 전부 읽어서 구글 Gemini(AI)에게 "핵심만 간결하게" 요약해달라고 요청합니다. 본문이 비어있으면 "요약"/"체크할것" 속성을 대신 쓰고, 그마저도 없으면 요약 줄 자체가 빠집니다. AI 요약이 실패하면(일시적 오류 등) 다듬어지지 않은 원문이라도 그대로 보내서, 알림 자체가 안 가는 일은 없게 합니다.
...
### 5. 구글 Gemini API 키 발급

1. https://aistudio.google.com 접속 후 구글 계정으로 로그인
2. "Get API key" → "Create API key" 클릭
3. 발급받은 키(`AIza...`로 시작)가 `GEMINI_API_KEY`
4. 무료 티어로 충분한 사용량이라 결제 수단 등록이 필요 없음 (다만 무료 티어는 보낸 데이터가 구글 모델 개선에 활용될 수 있음 — 사내 CS 로그를 사용하는 만큼 참고)
