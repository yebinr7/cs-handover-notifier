...
### 7. state.json 시딩하기
이 리포지토리는 배포 시점에 시딩된 `state.json`을 커밋해서 추적하고 있습니다. 다만 이 파일은 저절로 생기지 않으므로, **새로 배포하거나(포크 등) `last_checked` 값을 재설정해야 할 때는 아래 명령으로 다시 시딩해야 합니다.** 이 파일이 없으면 봇은 동작하지 않습니다.
